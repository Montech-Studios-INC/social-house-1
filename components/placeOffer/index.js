...
const PlaceOffer = ({id, contract}) => {
  ...
  const [previousOfferAmount, setPreviousOffer] = useState(0)
  ...
  const handlePlaceOffer = async (e, auctionId, amount, setLoading) => {
    ...
    e.preventDefault();
    setLoading(true);
    if(amount >= previousOfferAmount){
      if(parseFloat(formatUnits(etherBalance)).toFixed(3) < previousOfferAmount){
        notification['error']({
          message: 'Error Placing Offer!',
          description:
        ...
        try{
        const auction = await auctionHouse.createOffer(auctionId, parseEther(amount.toString()));
        setOfferPlaced(true);
        notification['success']({
          message: 'Offer Placed',
          description:
            'Successfully placed a Offer!',
        });
        setLoading(false)
      }
      catch (error){
        notification['error']({
          message: 'error placing an offer',
          description:
            error.message,
        });
        setLoading(false)
      ...
      notification['error']({
        message: 'Error Placing Offer!',
        description:
          'The new Offer must be 5% more than the current Offer!',
      });
      setLoading(false)
    }
  ...
  let newOffer;
  useEffect(()=>{
    const highestOffer = data?.pricing?.reserve?.current.highestBid?.pricing.amount ? parseFloat(parseFloat(formatEther(data?.pricing?.reserve?.current.highestBid?.pricing.amount)).toFixed(3)) : 0;
     newOffer = (highestOffer * 5) / 100 + highestOffer;
    setPreviousOffer(newOffer)
  }, [tokenData, data])
  useEffect(()=>{
    setAmount(newOffer)
  }, [newOffer, previousOfferAmount])
  ...
  //     setPreviousOffer(parseFloat(formatEther(data?.pricing?.reserve?.current.highestOffer?.pricing.amount)).toFixed(3))
  //     console.log(previousOfferAmount)
  // }
         ...
         <div className="bg-white px-10 py-4 w-1/2">
                    {!isloggedin && (
                        <div className="mt-20 flex flex-col justify-around items-center">
                            <p className="text-4xl font-bold text-center my-4 w-1/2">Connect your wallet to place a Offer</p>
                    ...
                    {isloggedin && (
                        <div className="mt-20 flex flex-col justify-around items-center">
                            <div className="flex flex-row justify-between">
                                <p className="text-xl">Place a Offer</p>
                                <p className="mx-4 mt-1">Your balance:  {etherBalance && parseFloat(formatEther(etherBalance)).toFixed(3)} ETH</p>
                            </div>
                            <div className="my-3">
                            <input className={`"bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none ${amount >= previousOfferAmount ? 'focus:border-purple-500' : 'focus:border-red-400'} focus:bg-white "`} id="inline-full-name" type="number" value={amount} onChange={(e)=>{setAmount(e.target.value)}} />
                            </div>
                            <p>Your Offer must be atleast ${previousOfferAmount}</p>
                            <p>The next Offer must be atleast 5% of the current Offer </p>
                            <button className={`bg-black text-white font-bold rounded px-4 py-2 outline-none w-full mt-2 ${amount < previousOfferAmount ? 'bg-gray-600 cursor-not-allowed' : ''}`} onClick={(e)=>{amount >= previousOfferAmount && handlePlaceOffer(e, tokenData?.token[0]?.nft?.auctionData?.id, amount, setLoading)}}>{!loading ? (
                            "Place a Offer"
                          ) : (
                            <Row justify='center'>
                              <img
                            ...
                            <p>You cannot withdraw a Offer once submitted</p>
                        </div>
                    )}
         </div>
     </div>
